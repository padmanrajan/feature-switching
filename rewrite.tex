...
\section{Abstract}
\label{sec:abstract}
In the speaker verification task, the test utterance has to be verified against the claim, which comes along with it. Feature fusion, score fusion and feature switching are different archetypes employed in the task of performing speaker verification. Feature fusion combines different features in order to use the complementary information present in it. This leads to very large dimension of features. An alternate approach is score fusion, also known as late fusion, where fusion is carried out at score-level and the contribution of feature types are determined by set of weights. The other significant way to attain the advantage of different feature space is feature switching, where the best discriminating feature is identified and used for every speaker. A comparative study of feature switching in speaker verification task is elaborated in this paper on the standard NIST SRE 2010 database for both male and female genders using Mel-Frequency Cepstral Coefficients  (MFCC) and Modified Group Delay features  (MGD) in two different frameworks namely, state-of-the-art i-vector framework and conventional UBM-GMM framework.
...
%	\begin{figure}
    \includegraphics[scale=0.48]{figures/tecer_mgd.eps}
	\caption*{(a)}
	\label{fig:GmmMgdOpt}
%	\end{figure}
\end{minipage}%
...
\centering  
%	\begin{figure}
    \includegraphics[scale=0.23]{figures/tecer_mfcc.eps}
	\caption*{(b)}
	\label{fig:GmmMfcOpt}
%	\end{figure}
\end{minipage}
...
In this paper, the experiments are conducted for both male and female genders. The MFCC and MGD features are extracted from both development and evaluation data, for both genders as mentioned in Section \ref{sec:featExt}.

In UBM-GMM framework Universal Background Model (UBM) is built for both genders in each feature domain. Speaker dependent models are built for training data, by adapting the means of the feature-based, gender-dependent UBM. Optimal feature for every train speaker in evaluation data is identified and a look-up table is generated as detailed in the Section \ref{subsec:ubm_optFeat}. The complete procedure of selecting optimal feature is shown in the Figure \ref{fig:fs_ubmgmm}.

\begin{figure}[h!tb]
\includegraphics[scale=0.25]{figures/opt_feat.eps}
\caption{Optimal Feature Selection Procedure in UBMGMM Framework}
\label{fig:fs_ubmgmm}
\end{figure}

In test phase, the test utterance comes with a claim. The optimal feature for the claim is identified from the look-up table and that optimal feature is extracted for given test utterances. With the help of UBM, these features are compared against the trained speaker-dependent models and log-likelihood scores are obtained. This scores are then subjected to T-Normalisation. Based on the ground truth given in database, the scores of test utterances are classified as true scores or impostor scores. EER is computed with these scores to evaluate the performance of the developed system. 

In i-vector framework, after building UBM, first order statistics are calculated for the development data, from which super vectors are formed. A randomly  initialized T-matrix is used along with super vectors to get the i-vectors for every trial in development data. After random initialization, T-matrix and i-vectors are re-estimated iteratively. The re-estimation is stopped with after few iterations, because the values of i-vectors obtained from next iteration do not differ much. The i-vectors of training data set are subjected to whitening transformation, followed by Linear Discriminant Analysis (LDA) and With-in Class Covariance Matrix (WCCN), to get completely decorrelated i-vectors. Then i-vectors for evaluation data set is generated using the T-matrix. Optimal features for training speakers are identified and a look up table is generated as in the Section \ref{subsec:ivec_optFeat}. The sketch of optimal feature selection procedure in i-vector framework is shown in the Figure \ref{fig:fs_ivec}. In the figure, Equations \ref{eq:sp} and \ref{eq:p_hat} is represented as $\Sigma/N$ and Max\{S\_mfcc,S\_mgd\} respectively.
...
For a given test utterance, i-vector is extracted in the claimed speaker's optimal feature domain. Either cosine similarity or Euclidean distance measures can be computed between the i-vectors of test utterance and the i-vectors of all the training utterances of claimed speaker. Scores are subjected to T-normalization, and the EER is computed for given threshold. True and imposter scores are identified based on ground truth. 
...
Similar to UBM-GMM systems, 38 dimensional feature vectors are extracted and feature-based 1024 mixture UBMs are built for each gender. i-vectors of reduced dimension $R$ are obtained from the T-matrix of size $(38\times1024)$-by-$R$. Whitening transformation and WCCN are applied to the i-vectors of development data using a transformation matrices of dimension $R$-$by$-$R$.
...
Performance of the baseline systems are evaluated by calculating EER between the impostor scores and true scores of each system and the results are listed in the Table \ref{tab:eer}.

\subsection{Score-Fusion Systems}
\label{subsec:scoreFusion}
	It is shown in literature that instead of using a single feature based speaker verification system, more than one feature can be used to improve the effectiveness of the system. This is possible with different strategies like early fusion, score fusion etc. {\it Feature fusion}, also often referred as {\it early fusion} is the process of concatenating the individual feature vectors of different feature types \cite{padmanInterspeech2010}. The detriment of this method is the increased dimension of the new concatenated feature vectors. 
	...
	In this paper, score fusion systems are built in i-vector and UBM-GMM frameworks for comparing the performance of feature switching systems. The four score fusion system are
	\begin{enumerate}
	\item SF-UBM-male
	\item SF-UBM-female
	\item SF-ivec-male
	\item SF-ivec-female	
	\end{enumerate}
	The EER's of these systems are listed in table \ref{tab:eer}.
